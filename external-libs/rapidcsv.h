...
struct ConverterParams {
   ...
   * numbers.
   * @param   pNumericLocale        specifies whether to honor LC_NUMERIC locale
   * (default true).
   */
  explicit ConverterParams(
      ...
      const long long pDefaultInteger = 0, const bool pNumericLocale = true)
...
public:
  ...
  void ToStr(const T &pVal, std::string &pStr) const {
    if (typeid(T) == typeid(int) || typeid(T) == typeid(short) ||
        typeid(T) == typeid(unsigned short) || typeid(T) == typeid(long) ||
  ...
  void ToVal(const std::string &pStr, T &pVal) const {
    try {
      if (typeid(T) == typeid(int)) {
        ...
        const bool isNegative = (ull != 0) && (signPos != std::string::npos) &&
                                (pStr.at(signPos) == '-');
...
public:
  ...
  explicit Document(
      ...
      : mPath(pPath), mLabelParams(pLabelParams),
        ...
        mLineReaderParams(pLineReaderParams), mData(), mColumnNames(),
        mRowNames() {
    if (!mPath.empty()) {
      ReadCsv();
    }
  ...
  /**
   * @brief   Constructor
   * @param   pStream               specifies a binary input stream to read CSV
   * data from.
   * @param   pLabelParams          specifies which row and column should be
   * treated as labels.
   * @param   pSeparatorParams      specifies which field and row separators
   ...
   * @param   pStream               specifies a binary output stream to write
   * the data to.
   */
  void Save(std::ostream &pStream) const { WriteCsv(pStream); }
  ...
  /**
   * @brief   Get column index by name. The result is undefined if the document
   * contains multiple columns sharing the same label name. Column indices
   * exceeding INT_MAX are not supported.
  ...
  int GetColumnIdx(const std::string &pColumnName) const {
    if (mLabelParams.mColumnNameIdx >= 0) {
      if (mColumnNames.find(pColumnName) != mColumnNames.end()) {
        return static_cast<int>(mColumnNames.at(pColumnName)) -
  ...
  void SetColumn(const size_t pColumnIdx, const std::vector<T> &pColumn) {
    ...
    for (auto itRow = pColumn.begin(); itRow != pColumn.end(); ++itRow) {
      ...
      mData
          .at(static_cast<size_t>(std::distance(pColumn.begin(), itRow) +
                                  mLabelParams.mColumnNameIdx + 1))
    ...
    for (auto itRow = mData.begin(); itRow != mData.end(); ++itRow) {
      if (std::distance(mData.begin(), itRow) >= mLabelParams.mColumnNameIdx) {
        if (dataColumnIdx < itRow->size()) {
          ...
          const std::string errStr =
              "column out of range: " + std::to_string(pColumnIdx) +
              " (on row " +
              std::to_string(std::distance(mData.begin(), itRow)) + ")";
          throw std::out_of_range(errStr);
        }
  ...
  void InsertColumn(const size_t pColumnIdx,
    ...
    if (pColumn.empty()) {
      ...
      for (auto itRow = pColumn.begin(); itRow != pColumn.end(); ++itRow) {
        ...
        converter.ToStr(*itRow, str);
        const size_t rowIdx =
            static_cast<size_t>(std::distance(pColumn.begin(), itRow) +
                                (mLabelParams.mColumnNameIdx + 1));
    ...
    while (column.size() > GetDataRowCount()) {
      ...
      const size_t columnCount =
          std::max<size_t>(static_cast<size_t>(mLabelParams.mColumnNameIdx + 1),
                           GetDataColumnCount());
  ...
  size_t GetColumnCount() const {
    ...
    const std::ptrdiff_t count =
        static_cast<std::ptrdiff_t>(
            (mData.size() > firstRow) ? mData.at(firstRow).size() : 0) -
        (mLabelParams.mRowNameIdx + 1);
    return (count >= 0) ? static_cast<size_t>(count) : 0;
  }

  /**
   * @brief   Get row index by name. The result is undefined if the document
   * contains multiple rows sharing the same label name. Row indices exceeding
  ...
  int GetRowIdx(const std::string &pRowName) const {
    if (mLabelParams.mRowNameIdx >= 0) {
      if (mRowNames.find(pRowName) != mRowNames.end()) {
        return static_cast<int>(mRowNames.at(pRowName)) -
               (mLabelParams.mColumnNameIdx + 1);
      }
    }
    return -1;
  ...
  template <typename T> std::vector<T> GetRow(const size_t pRowIdx) const {
    ...
    std::vector<T> row;
    Converter<T> converter(mConverterParams);
    for (auto itCol = mData.at(dataRowIdx).begin();
         itCol != mData.at(dataRowIdx).end(); ++itCol) {
      if (std::distance(mData.at(dataRowIdx).begin(), itCol) >
  ...
  void SetRow(const size_t pRowIdx, const std::vector<T> &pRow) {
    ...
    for (auto itCol = pRow.begin(); itCol != pRow.end(); ++itCol) {
      ...
      mData.at(dataRowIdx)
          .at(static_cast<size_t>(std::distance(pRow.begin(), itCol) +
  ...
  void RemoveRow(const size_t pRowIdx) {
    ...
    if (dataRowIdx < mData.size()) {
      ...
      const std::string errStr = "row out of range: " + std::to_string(pRowIdx);
      throw std::out_of_range(errStr);
    }
  ...
  void InsertRow(const size_t pRowIdx,
    ...
    if (pRow.empty()) {
      ...
      for (auto itCol = pRow.begin(); itCol != pRow.end(); ++itCol) {
        std::string str;
        converter.ToStr(*itCol, str);
        row.at(static_cast<size_t>(std::distance(pRow.begin(), itCol) +
                                   mLabelParams.mRowNameIdx + 1)) = str;
      }
    }
  ...
  T GetCell(const std::string &pColumnName, const std::string &pRowName) const {
    ...
    return GetCell<T>(static_cast<size_t>(columnIdx),
  ...
  T GetCell(const std::string &pColumnName, const std::string &pRowName,
    ...
    return GetCell<T>(static_cast<size_t>(columnIdx),
                      static_cast<size_t>(rowIdx), pToVal);
  }
  ...
  void SetCell(const std::string &pColumnName, const std::string &pRowName,
    ...
    SetCell<T>(static_cast<size_t>(columnIdx), static_cast<size_t>(rowIdx),
               pCell);
  }
   ...
   */
  template <typename T>
  void SetCell(const size_t pColumnIdx, const std::string &pRowName,
               const T &pCell) {
    const int rowIdx = GetRowIdx(pRowName);
    if (rowIdx < 0) {
      throw std::out_of_range("row not found: " + pRowName);
   ...
   */
  template <typename T>
  void SetCell(const std::string &pColumnName, const size_t pRowIdx,
               const T &pCell) {
    const int columnIdx = GetColumnIdx(pColumnName);
    if (columnIdx < 0) {
      throw std::out_of_range("column not found: " + pColumnName);
  ...
  std::string GetColumnName(const size_t pColumnIdx) const {
    ...
    return mData.at(static_cast<size_t>(mLabelParams.mColumnNameIdx))
        .at(dataColumnIdx);
  }
  ...
  void SetColumnName(size_t pColumnIdx, const std::string &pColumnName) {
    ...
    if ((nameRowIdx < mData.size()) &&
        (dataColumnIdx < mData.at(nameRowIdx).size())) {
    ...
    mData.at(static_cast<size_t>(mLabelParams.mColumnNameIdx))
        .at(dataColumnIdx) = pColumnName;
  }
  ...
  std::vector<std::string> GetColumnNames() const {
    if (mLabelParams.mColumnNameIdx >= 0) {
      const std::vector<std::string> &labelRow =
          mData.at(static_cast<size_t>(mLabelParams.mColumnNameIdx));
  ...
  std::string GetRowName(const size_t pRowIdx) const {
    ...
    return mData.at(dataRowIdx)
        .at(static_cast<size_t>(mLabelParams.mRowNameIdx));
  }
  ...
  void SetRowName(size_t pRowIdx, const std::string &pRowName) {
    ...
    if ((mLabelParams.mRowNameIdx >= 0) && (dataRowIdx < mData.size()) &&
        (static_cast<size_t>(mLabelParams.mRowNameIdx) <
         mData.at(dataRowIdx).size())) {
    ...
    mData.at(dataRowIdx).at(static_cast<size_t>(mLabelParams.mRowNameIdx)) =
        pRowName;
  }
  ...
  std::vector<std::string> GetRowNames() const {
    ...
    if (mLabelParams.mRowNameIdx >= 0) {
      for (auto itRow = mData.begin(); itRow != mData.end(); ++itRow) {
        if (std::distance(mData.begin(), itRow) > mLabelParams.mColumnNameIdx) {
          rownames.push_back(
              itRow->at(static_cast<size_t>(mLabelParams.mRowNameIdx)));
        }
      }
    }
    return rownames;
...
private:
  ...
  void ReadCsv(std::istream &pStream) {
    ...
    if ((bom2b == bomU16le) || (bom2b == bomU16be)) {
      ...
      const std::string utf8 = Utf16ToUtf8(
          buffer.data() + 2, static_cast<size_t>(length) - 2, mIsLE);
  ...
  static inline void CheckMaxCount(const size_t pCount) {
    ...
    static_assert(RAPIDCSV_MAX_COUNT > 0,
                  "RAPIDCSV_MAX_COUNT must be a positive value");
    static const size_t s_MaxCount = static_cast<size_t>(RAPIDCSV_MAX_COUNT);
    if (pCount > s_MaxCount) {
      const std::string errStr =
          "max count exceeded: " + std::to_string(s_MaxCount);
      throw std::out_of_range(errStr);
    }
#else
    (void)pCount;
  ...
  void ParseCsv(std::istream &pStream, std::streamsize p_FileLength) {
    ...
    while (p_FileLength > 0) {
      const std::streamsize toReadLength =
          std::min<std::streamsize>(p_FileLength, bufLength);
      pStream.read(buffer.data(), toReadLength);

      // With user-specified istream opened in non-binary mode on windows, we
      // may have a data length mismatch, so ensure we don't parse outside
      ...
      for (size_t i = 0; i < static_cast<size_t>(readLength); ++i) {
        if (buffer[i] == mSeparatorParams.mQuoteChar) {
          if (cell.empty() || (cell[0] == mSeparatorParams.mQuoteChar)) {
            ...
            if (std::all_of(cell.begin(), firstQuote,
      ...
      for (auto itc = itr->begin(); itc != itr->end(); ++itc) {
        if (mSeparatorParams.mAutoQuote &&
            ((itc->find(mSeparatorParams.mSeparator) != std::string::npos) ||
             ...
             (itc->find('\n') != std::string::npos) ||
             (itc->find('\r') != std::string::npos))) {
          // escape quotes in string
          std::string str = *itc;
          const std::string quoteCharStr =
              std::string(1, mSeparatorParams.mQuoteChar);
  ...
  size_t GetDataColumnCount() const {
    const size_t firstDataRow = static_cast<size_t>(
        (mLabelParams.mColumnNameIdx >= 0) ? mLabelParams.mColumnNameIdx : 0);
  ...
  inline size_t GetDataRowIndex(const size_t pRowIdx) const {
    const size_t firstDataRow = static_cast<size_t>(
        (mLabelParams.mColumnNameIdx + 1 >= 0) ? mLabelParams.mColumnNameIdx + 1
                                               : 0);
    return pRowIdx + firstDataRow;
  }

  inline size_t GetDataColumnIndex(const size_t pColumnIdx) const {
    const size_t firstDataColumn = static_cast<size_t>(
        (mLabelParams.mRowNameIdx + 1 >= 0) ? mLabelParams.mRowNameIdx + 1 : 0);
  ...
  void UpdateColumnNames() {
    ...
    if ((mLabelParams.mColumnNameIdx >= 0) &&
      ...
      size_t i = 0;
      for (auto &columnName :
           mData[static_cast<size_t>(mLabelParams.mColumnNameIdx)]) {
  ...
  void UpdateRowNames() {
    ...
    if ((mLabelParams.mRowNameIdx >= 0) &&
      ...
      for (auto &dataRow : mData) {
        // rows too short to hold a row label are left unnamed, but must still
        // be counted
        if (static_cast<std::ptrdiff_t>(dataRow.size()) >
            mLabelParams.mRowNameIdx) {
  ...
  // Reads the UTF-16 code unit at byte offset pIdx, which must be a valid
  // offset of a complete code unit.
  ...
  static void AppendUtf16(uint32_t pCodePoint, bool pIsLE,
                          std::string &pUtf16) {
  ...
  // Converts UTF-16 encoded data (without byte order mark) to UTF-8. Surrogate
  // pairs are combined into a single code point. Unpaired surrogates and a
  // trailing odd byte are replaced with U+FFFD.
  ...
  // Converts UTF-8 encoded data to UTF-16, prefixed with a byte order mark.
  // Code points outside the basic multilingual plane are encoded as surrogate
  // pairs. Malformed input is replaced with U+FFFD.
    ...
    while (idx < pUtf8.size()) {
      ...
      if ((seqLen > 1) && ((idx + seqLen) <= pUtf8.size())) {
        for (size_t byteIdx = 1; byteIdx < seqLen; ++byteIdx) {
          const uint32_t contByte =
              static_cast<unsigned char>(pUtf8[idx + byteIdx]);
